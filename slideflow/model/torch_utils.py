...
def concordance_index(y_true: torch.Tensor, y_pred: torch.Tensor) -> torch.Tensor:
    """Calculate concordance index (C-index).

    Args:
        y_true (torch.Tensor): True labels (durations and events).
        y_pred (torch.Tensor): Predictions (log hazard ratios).
    ...
    durations = y_true[:, 0]
    events = y_true[:, 1]
    log_hr = y_pred.squeeze()

    # Negative of log hazard ratio to have correct relationship with survival
    log_hr = -log_hr

    # Compute pairwise comparisons
    g = torch.sub(log_hr.unsqueeze(-1), log_hr.unsqueeze(0))
    g = (g == 0).float() * 0.5 + (g > 0).float()

    # Compute pairwise differences in durations
    f = torch.sub(durations.unsqueeze(-1), durations.unsqueeze(0)) > 0

    # Compute pairwise products of event indicators
    event = torch.mul(events.unsqueeze(-1), events.unsqueeze(0))

    # Multiply pairwise differences and event products
    f = torch.mul(f.float(), event.float())

    # Set diagonal and upper triangle to zero
    f = torch.triu(f, diagonal=1)

    # Compute numerator and denominator of C-index
    g = torch.sum(torch.mul(g, f))
    f = torch.sum(f)

    # Compute C-index
    return torch.where(f == 0, torch.tensor(0.0), g / f)
...
class CoxProportionalHazardsLoss(torch.nn.modules.loss._Loss):
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        ...
        events = events.view(-1)  # Make sure events is a 1D tensor
        log_h = log_h.view(-1)
